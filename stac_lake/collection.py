...
class BaseItemRequirement:
    def validate_item_against_requirement(self, item: pystac.Item) -> ItemRequirementValidationResult:
...
class ProjectionRequirement(BaseItemRequirement):
    ...
    def validate_item_against_requirement(self, item: pystac.Item) -> ItemRequirementValidationResult:
        ...
        if self.epsg and _get_epsg_code_int(item) != self.epsg:
            return ItemRequirementValidationResult(False, "Item does not match required EPSG code.")
...
class AssetRequirement(BaseItemRequirement):
    ...
    def validate_item_against_requirement(self, item: pystac.Item) -> ItemRequirementValidationResult:
        ...
        if asset is None:
            return ItemRequirementValidationResult(False, f"Item is missing required asset: {self.asset_name}")
        
        if self.media_type and asset.media_type != self.media_type:
            return ItemRequirementValidationResult(False, f"Asset '{self.asset_name}' does not match required media type: {self.media_type}")
...
def validate_item_against_requirements(item: pystac.Item, requirements: List[BaseItemRequirement]) -> ItemRequirementValidationResult:
    message = ""
    is_valid = True
